		...
		for (const auto& ware_index: index_list) {
			if (map.count(ware_index) == 1) {
				...
				result += (boost::format("<div width=30 padding=2><p align=center>"
				       "<div width=26 background=%s><p align=center><img src=\""
				       "%s"
				       "\"></p></div><div width=26 background=%s><p>%s</p></div></p></div>") %
						   style.icon_background().hex_value() %
						   tribe.get_ware_descr(ware_index)->icon_filename() %
						   style.info_background().hex_value() %
						   style.info_font().as_font_tag(get_amount_string(amount))).str();
			}
		}
	}
	return result;
